  ...
  <div class="container">
    ...
    <div class="order" v-for="value in orderlist">
      <div class="order-title">
        <span>{{value.orderAddress.address}}</span>
        <p class="itemactive">{{value.orderStatus}}</p>
      </div>
      <div class="order-time">
        <span>{{orderTime(value)}}</span>
      </div>
      <div class="order-list">
        <div class="order-detail">
          ...
          <div class="order-name">{{value.orderName}}
          <p>{{value.orderExplain}}</p></div>
          <div class="order-price">￥{{value.orderPrice}}
          <span>x{{value.orderCount}}</span></div>
        </div>
        <div class="order-summary">
        <span>共{{value.orderCount}}件商品，合计：￥{{value.orderPrice}} 含运费(￥0.00)</span>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      orderPic (value) {
        if (value.orderPic) {
          return value.orderPic.thumbnailURL(700, 600)
        }
      }
    },
    mounted () {
      dataOrder.queryOrder().then(res => {
